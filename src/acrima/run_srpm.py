...
class ResNet18(nn.Module):
    ...
    def __init__(self, num_classes=2, in_channels=3):
        ...
        self.layer1 = self._make_layer(22, 2, stride=1)
        self.layer2 = self._make_layer(44, 2, stride=2)
        self.layer3 = self._make_layer(88, 2, stride=2)
        self.layer4 = self._make_layer(176, 2, stride=2)
...
class ACRIMADataset(Dataset):
    ...
    @property
    def imgs(self):
        """Lazy-load all images into memory."""
...
def prepare_data(seed, num_batches):
    """Prepare ACRIMA data with multi-augmentation support."""
    print("\n" + "="*60)
    ...
    if not train_dir.exists():
        raise FileNotFoundError(
            ...
            f"https://www.kaggle.com/datasets/toaharahmanratul/acrima-dataset\n"
    ...
    # From train_indices, create labeled/unlabeled split
    n_train = len(train_indices)
    n_labeled = int(n_train * LABELED_FRACTION)
    
    rng = random.Random(seed)
    shuffled_train = train_indices.copy()
    rng.shuffle(shuffled_train)
    
    labeled_indices = shuffled_train[:n_labeled]
    unlabeled_indices = shuffled_train[n_labeled:]
...
def run_upper_bound_experiment(data_dict):
    ...
    labeled_indices = data_dict["labeled_indices"]
    ...
    print(f"\nTraining AUGMENTATION ENSEMBLE with 100% labeled data ({len(all_indices)} samples)...")
    ...
    upper_bound_metrics = {}
    for name in ["ResNet18_Pure", "ResNet18_Noise", "ResNet18_Geometric", "Ensemble"]:
...
def print_final_results(results, upper_bound_metrics=None):
    ...
    for model_name in ["ResNet18_Pure", "ResNet18_Noise", "ResNet18_Geometric", "Ensemble"]:
...
def main():
    ...
    try:
        ...
        print(f"Dataset: ACRIMA - Glaucoma Detection (Fundus Images)")
        print(f"Image Size: {IMG_SIZE}x{IMG_SIZE} RGB | Confidence >= 0.75")
        print("Architecture: ResNet18 [22, 44, 88, 176] (identical for all 3 models)")
        print("Diversity: Different augmentation strategies (Pure, Noise, Geometric)")
        ...
        print(f"Treshold: {CONFIDENCE_THRESHOLD}")
