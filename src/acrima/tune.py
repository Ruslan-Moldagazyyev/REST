"""
Augmentation-Based Joint Ensemble SRPM-ST TUNING for ACRIMA: Glaucoma Detection
================================================================================

Adapted from Skin Cancer version for ACRIMA glaucoma fundus image dataset.

KEY CHANGES FROM SKIN CANCER:
1. Data loading: Parse filename for labels (Im###_g_ACRIMA = glaucoma, Im###_ACRIMA = normal)
2. Data splitting: Single folder → stratified train/val/test splits
3. Class names: normal=0, glaucoma=1
4. Smaller dataset: 705 images total → adjusted mini-batch range

Dataset characteristics (ACRIMA):
    - 705 fundus images total
    - 396 glaucomatous images (labeled with '_g_' in filename)
    - 309 normal images (no '_g_' in filename)
    - Acquired with Topcon TRC retinal camera, 35° field of view
    - Annotations by two glaucoma specialists (8 years experience)

Download from:
    - Figshare: https://figshare.com/articles/dataset/CNNs_for_Automatic_Glaucoma_Assessment_using_Fundus_Images_An_Extensive_Validation/7613135
    - Kaggle: https://www.kaggle.com/datasets/toaharahmanratul/acrima-dataset

Expected folder structure after download:
    DATA_ROOT/
    └── Images/
        ├── Im001_ACRIMA.jpg      (normal)
        ├── Im002_g_ACRIMA.jpg    (glaucoma)
        └── ...
...
Diversity Source: Different augmentation strategies
  ...
  - Model 2: Noise/Artifact (Gaussian noise, salt-pepper, blur - NOT color jitter)
...
OUTPUT_DIR = f"aug_ensemble_srpm_st_results_acrima_{SEED}_{CONFIDENCE_THRESHOLD}"
...
class ACRIMADataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', transform=None):
        ...
        if len(self.labels) > 0:
            ...
            print(f"  {split.capitalize()}: Found {len(self.image_paths)} images")
            print(f"    Glaucoma: {glaucoma_count}, Non-Glaucoma: {normal_count}")
        else:
...
class JointAugmentationEnsemble(nn.Module):
    ...
    def __init__(self, num_classes=2, in_channels=3):
        ...
        self.model_names = ["ResNet18_Pure", "ResNet18_Noise", "ResNet18_Geometric"]
        self.augmentation_types = ["pure", "noise", "geometric"]
...
def compute_class_weights(dataset_raw, indices):
    ...
    for c in range(NUM_CLASSES):
        class_name = "normal" if c == 0 else "glaucoma"
...
def prepare_data():
    ...
    if not train_dir.exists():
        raise FileNotFoundError(
            ...
            f"Please download the ACRIMA dataset from:\n"
            f"  Kaggle: https://www.kaggle.com/datasets/toaharahmanratul/acrima-dataset\n"
    ...
    print(f"\nDataset: ACRIMA - Glaucoma Detection (Binary, {NUM_CLASSES} classes)")
    print(f"Total training images: {n_train_total}")
    print(f"  After val split: {n_train} for train, {len(val_indices)} for val")
    print(f"  Training set: {normal_count} non-glaucoma, {glaucoma_count} glaucoma")
    ...
    print("  Model 2: Noise/Artifact (Gaussian, salt-pepper, blur)")
...
def evaluate_joint_ensemble(ensemble: JointAugmentationEnsemble, loader: DataLoader) -> Dict:
    ...
    for i, name in enumerate(["ResNet18_Pure", "ResNet18_Noise", "ResNet18_Geometric"]):
...
def run_joint_ensemble_srpm_st(
    ...
    print(f"    Pseudo-label accuracy: {correct}/{len(pseudo_labels_dict)} ({pseudo_accuracy*100:.1f}%)")
    print(f"    Class distribution: Class 0 (normal): {pseudo_class_0}, Class 1 (glaucoma): {pseudo_class_1}")
    ...
    return {
        ...
        "pseudo_class_0": pseudo_class_0,
        "pseudo_class_1": pseudo_class_1,
        "resnet_pure_test_auc": test_metrics["ResNet18_Pure_auc"],
        "resnet_noise_test_auc": test_metrics["ResNet18_Noise_auc"],
...
def save_results(all_results: List[Dict]):
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        writer.writerow([
            ...
            'resnet_pure_auc', 'resnet_noise_auc', 'resnet_geometric_auc'
        ])
        for r in all_results:
            if "error" not in r:
                writer.writerow([
                    ...
                    f"{r['resnet_pure_test_auc']:.4f}", f"{r['resnet_noise_test_auc']:.4f}",
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("Dataset: ACRIMA - Glaucoma Detection (Fundus Images)\n")
        ...
        f.write(f"Validation Split: {VAL_SPLIT_RATIO*100:.0f}% of training data\n")
        ...
        f.write("  - Model 2: Noise/Artifact (Gaussian, salt-pepper, blur)\n")
        ...
        if valid_results:
            ...
            f.write(f"  ResNet18_Noise AUC:          {best['resnet_noise_test_auc']:.4f}\n")
...
def print_final_summary(all_results: List[Dict]):
    ...
    print("Dataset: ACRIMA - Glaucoma Detection")
    ...
    if valid_results:
        ...
        print(f"    ResNet18_Noise (artifact aug):    {best['resnet_noise_test_auc']:.4f}")
        print(f"    ResNet18_Geometric (spatial aug): {best['resnet_geometric_test_auc']:.4f}")


def main():
    print("\n" + "="*80)
    print("AUGMENTATION-BASED JOINT ENSEMBLE SRPM-ST")
    print("Dataset: ACRIMA - Glaucoma Detection (Fundus Images)")
