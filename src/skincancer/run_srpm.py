...
class RandomSharpness:
    """Randomly adjust sharpness of tensor image."""
    def __init__(self, factor_range=(0.5, 1.5)):
        self.factor_range = factor_range
    
    def __call__(self, tensor):
        # Simple sharpness via high-pass filtering approximation
        factor = random.uniform(*self.factor_range)
        if factor == 1.0:
            return tensor
        # Create a simple blur and subtract for sharpening
        blur_kernel = torch.ones(1, 1, 3, 3, device=tensor.device) / 9.0
        c = tensor.shape[0]
        blurred = F.conv2d(
            tensor.unsqueeze(0), 
            blur_kernel.expand(c, 1, 3, 3),
            padding=1,
            groups=c
        ).squeeze(0)
        # Sharpen: original + factor * (original - blurred)
        sharpened = tensor + (factor - 1.0) * (tensor - blurred)
...
def get_augmentation_transforms():
    ...
    transform_geometric = transforms.Compose([
        ...
        # Full rotation - skin lesions can appear at any angle
        transforms.RandomRotation(degrees=180),
        # Flips
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomVerticalFlip(p=0.5),
        # Mild affine for perspective variations
...
class SkinCancerDataset(Dataset):
    ...
    def imgs(self):
        if self._imgs_cache is None:
            ...
            # Keep as list, not numpy array, to avoid object dtype issues
...
def prepare_data(seed, num_batches):
    """Prepare Skin Cancer data with multi-augmentation support."""
    print("\n" + "="*60)
...
def run_upper_bound_experiment(data_dict):
    ...
    # All training indices (labeled + unlabeled)
...
def main():
    ...
    SEEDS = [42, 43, 44]
    NUM_BATCHES = [28, 34, 30]  # Update with optimal values from tuning
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = f"results_aug_ensemble_skincancer/{timestamp}"
    ...
    try:
        ...
        print(f"Dataset: Skin Cancer: Malignant vs. Benign (Kaggle)")
